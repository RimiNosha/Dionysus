        ...
        <Section>
          ...
          <LabeledList>
            <LabeledList.Item label="Prefix">
              {PREFIXES.map((prefix) => (
                <Button key={prefix.toLowerCase()} content={prefix} disabled={data.currentSection !== 1} onClick={() => act(prefix + ' ')} />
              ))}
            </LabeledList.Item>
            <LabeledList.Item label="Title">
              {TITLES.map((title) => (
                <Button key={title.toLowerCase()} content={title} disabled={data.currentSection > 2} onClick={() => act(title + ' ')} />
              ))}
            </LabeledList.Item>
            ...
            <LabeledList.Item label="Suffix">
              {SUFFIXES.map((suffix) => (
                <Button key={suffix.toLowerCase()} content={suffix} disabled={data.currentSection !== 4} onClick={() => act(' ' + suffix)} />
              ))}
            </LabeledList.Item>
