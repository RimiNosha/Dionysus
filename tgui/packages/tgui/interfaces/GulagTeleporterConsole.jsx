...
export const GulagTeleporterConsole = (props) => {
  ...
  const { teleporter, teleporter_lock, teleporter_state_open, teleporter_location, beacon, beacon_location, id, id_name, can_teleport, goal = 0, prisoner = {} } = data;
  return (
    <Window width={350} height={295}>
      <Window.Content>
        <Section
          ...
          buttons={
            <>
              <Button content={teleporter_state_open ? 'Open' : 'Closed'} disabled={teleporter_lock} selected={teleporter_state_open} onClick={() => act('toggle_open')} />
              <Button
        ...
        >
          <LabeledList>
            ...
            <LabeledList.Item label="Receiver Beacon" color={beacon ? 'good' : 'bad'} buttons={!beacon && <Button content="Reconnect" onClick={() => act('scan_beacon')} />}>
        ...
        <Section title="Prisoner Details">
          <LabeledList>
            ...
            <LabeledList.Item label="Point Goal">
              <NumberInput value={goal} width="48px" minValue={1} maxValue={1000} onChange={(value) => act('set_goal', { value })} />
            </LabeledList.Item>
        ...
        <Button fluid content="Process Prisoner" disabled={!can_teleport} textAlign="center" color="bad" onClick={() => act('teleport')} />
      </Window.Content>
    </Window>
  );
};
