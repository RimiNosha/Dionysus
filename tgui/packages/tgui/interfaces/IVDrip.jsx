...
export const IVDrip = (props) => {
  ...
  const { transferRate, injectOnly, maxInjectRate, minInjectRate, mode, connected, beakerAttached, useInternalStorage } = data;
  return (
        ...
        <Section title="IV Status">
          <LabeledList>
            ...
            <LabeledList.Item label="Mode">
              <Button disabled={injectOnly} content={mode ? 'Injecting' : 'Draining'} icon={mode ? 'sign-in-alt' : 'sign-out-alt'} onClick={() => act('changeMode')} />
            </LabeledList.Item>
            <LabeledList.Item label="Attached Container" color={beakerAttached ? 'good' : 'average'}>
              ...
              <Button disabled={!beakerAttached || useInternalStorage} content="Eject" icon="eject" onClick={() => act('eject')} />
            </LabeledList.Item>
          </LabeledList>
        </Section>
