...
export const GasmixParser = (props: GasmixParserProps) => {
  const { gasmix, gasesOnClick, temperatureOnClick, volumeOnClick, pressureOnClick, ...rest } = props;
  ...
  return !total_moles ? (
    ...
    <LabeledList {...rest}>
      {gases.map((gas) => (
        <LabeledList.Item label={gasesOnClick ? <Button content={gas[1]} onClick={() => gasesOnClick(gas[0])} /> : gas[1]} key={gas[1]}>
          {gas[2].toFixed(2) + ' mol (' + ((gas[2] / total_moles) * 100).toFixed(2) + ' %)'}
        </LabeledList.Item>
      ))}
      <LabeledList.Item label={temperatureOnClick ? <Button content={'Temperature'} onClick={() => temperatureOnClick()} /> : 'Temperature'}>
      ...
      <LabeledList.Item label={pressureOnClick ? <Button content={'Pressure'} onClick={() => pressureOnClick()} /> : 'Pressure'}>
