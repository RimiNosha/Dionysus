...
const GasSearchBar = (props: { activeInput: boolean; onChange: (inputValue: string) => void; setActiveInput: (toggle: boolean) => void; title: ReactNode }) => {
...
const GasHandbook = (props) => {
  ...
  const [activeReactionId, setActiveReactionId] = useLocalState('activeReactionId', '');
  const [gasActiveInput, setGasActiveInput] = useLocalState('gasActiveInput', false);
    ...
    <Section
      title={
        <GasSearchBar
          ...
          onChange={(keyword) => setActiveGasId(gasInfo.find((gas) => gas.name.toLowerCase().startsWith(keyword.toLowerCase()))?.id || '')}
    ...
    >
      {relevantGas && (
        <>
          ...
          <Box mb="0.5em">{'Specific heat: ' + relevantGas.specific_heat + ' Joule/KelvinMol'}</Box>
          <Box mb="0.5em">{'Relevant Reactions:'}</Box>
          {Object.entries(relevantGas.reactions).map(([reaction_id, reaction_name]) => (
            <Box key={reaction_id} mb="0.5em">
              <Button onClick={() => setActiveReactionId(reaction_id)} content={reaction_name} />
            </Box>
          ))}
        </>
      )}
...
const ReactionHandbook = (props) => {
  ...
  const [activeGasId, setActiveGasId] = useLocalState('activeGasId', '');
  const [activeReactionId, setActiveReactionId] = useLocalState('activeReactionId', '');
  const [reactionActiveInput, setReactionActiveInput] = useLocalState('reactionActiveInput', false);
  const relevantReaction = reactionInfo?.find((reaction) => reaction.id === activeReactionId);
  return (
    <Section
      title={
        <GasSearchBar
          title={relevantReaction ? 'Reaction: ' + relevantReaction.name : 'Reaction Lookup'}
          onChange={(keyword) => setActiveReactionId(reactionInfo.find((reaction) => reaction.name.toLowerCase().startsWith(keyword.toLowerCase()))?.id || '')}
              ...
              <LabeledList.Item
                ...
                label={
                  factor.factor_type === 'gas' && factor.factor_id ? (
                    <Button onClick={() => setActiveGasId(String(factor.factor_id))} content={factor.factor_name} />
                  ) : factor.tooltip ? (
                    <Tooltip content={factor.tooltip} position="top">
                      <Flex>
                        <Flex.Item style={{ borderBottom: 'dotted 2px' }} shrink>
                          {factor.factor_name + ':'}
                        </Flex.Item>
                      </Flex>
                    </Tooltip>
