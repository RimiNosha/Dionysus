...
export const InsertedSkillchip = (props) => {
  ...
  const { skillchip_ready, slot_use, slots_used, slots_max, implantable_reason, implantable, complexity, skill_name, skill_desc, skill_icon, working } = data;
    ...
    <Section
      ...
      buttons={
        <>
          ...
          />
          <Button icon="eject" disabled={!!working} onClick={() => act('eject')} content="Eject" />
        </>
      }
    >
      <Stack fill align="center">
        ...
        <Stack.Item grow basis={0}>
          <LabeledList>
            ...
            {!!implantable_reason && (
              <LabeledList.Item label="Error" color={implantable ? 'good' : 'bad'}>
                {implantable_reason}
              </LabeledList.Item>
            )}
          </LabeledList>
...
export const ImplantedSkillchips = (props) => {
  ...
  const { slots_used, slots_max, complexity_used, complexity_max, working } = data;
        ...
        <Table>
          <Table.Row header>
            ...
            <Table.Cell textAlign="center">
              <Button color="transparent" icon="brain" tooltip="Complexity" tooltipPosition="top" content={complexity_used + '/' + complexity_max} />
            </Table.Cell>
            <Table.Cell textAlign="center">
              <Button color="transparent" icon="save" tooltip="Slot Size" tooltipPosition="top" content={slots_used + '/' + slots_max} />
            </Table.Cell>
            <Table.Cell textAlign="center">
              <Button color="transparent" icon="check" tooltip="Is Active" tooltipPosition="top" />
            </Table.Cell>
            <Table.Cell textAlign="center">
          ...
          {current.map((skill) => (
            <Table.Row key={skill.ref}>
              <Table.Cell>
                <Icon textAlign="center" width="18px" mr={1} name={skill.icon} />
                {skill.name}
              </Table.Cell>
              <Table.Cell bold color={(!!skill.active && 'good') || (skill.complexity + complexity_used > complexity_max && 'bad') || 'grey'} textAlign="center">
                {skill.complexity}
              </Table.Cell>
              ...
              <Table.Cell textAlign="center">
                <Icon name={skill.active ? 'check' : 'times'} color={skill.active ? 'good' : 'bad'} />
              </Table.Cell>
              <Table.Cell textAlign="center">{(skill.cooldown > 0 && Math.ceil(skill.cooldown / 10) + 's') || '0s'}</Table.Cell>
              <Table.Cell textAlign="center">
                <Button
        ...
        {!!working && (
          <NoticeBox danger>
            <Flex direction="column">
              <Flex.Item mb={0.5}>Operation in progress. Please do not leave the chamber.</Flex.Item>
              <Flex.Item>
