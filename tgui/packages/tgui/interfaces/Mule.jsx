...
export const Mule = (props) => {
  ...
  const { on, cell, cellPercent, load, mode, modeStatus, haspai, autoReturn, autoPickup, reportDelivery, destination, home, id, destinations = [] } = data;
    ...
    <Window width={350} height={425}>
      <Window.Content>
        ...
        <Section
          title="Status"
          minHeight="110px"
          buttons={!locked && <Button icon={on ? 'power-off' : 'times'} content={on ? 'On' : 'Off'} selected={on} onClick={() => act('on')} />}
        >
          ...
          >
            <LabeledList>
              ...
              <LabeledList.Item label="Destination">
                <Dropdown over selected={destination || 'None'} options={destinations} width="150px" onSelected={(value) => act('destination', { value })} />
                <Button icon="stop" content="Stop" onClick={() => act('stop')} />
                <Button icon="play" content="Go" onClick={() => act('go')} />
              </LabeledList.Item>
              <LabeledList.Item label="Home">
                <Dropdown over selected={home} options={destinations} width="150px" onSelected={(value) => act('destination', { value })} />
