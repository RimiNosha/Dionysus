...
export const RadioactiveMicrolaser = (props) => {
  ...
  return (
    <Window title="Radioactive Microlaser" width={320} height={335} theme="syndicate">
      <Window.Content>
        <Section>
          <LabeledList>
        ...
        <Section title="Scanner Controls">
          <LabeledList>
            <LabeledList.Item label="Irradiation">
              <Button icon={irradiate ? 'power-off' : 'times'} content={irradiate ? 'On' : 'Off'} selected={irradiate} onClick={() => act('irradiate')} />
            </LabeledList.Item>
            <LabeledList.Item label="Stealth Mode">
              <Button icon={stealth ? 'eye-slash' : 'eye'} content={stealth ? 'On' : 'Off'} disabled={!irradiate} selected={stealth} onClick={() => act('stealth')} />
            </LabeledList.Item>
