...
export const SmokeMachine = (props) => {
  ...
  const { TankContents, isTankLoaded, TankCurrentVolume, TankMaxVolume, active, setting, screen, maxSetting = [] } = data;
  return (
    <Window width={350} height={350}>
      <Window.Content>
        <Section title="Dispersal Tank" buttons={<Button icon={active ? 'power-off' : 'times'} selected={active} content={active ? 'On' : 'Off'} onClick={() => act('power')} />}>
          <ProgressBar
            ...
            <LabeledList>
              <LabeledList.Item label="Range">
                {[1, 2, 3, 4, 5].map((amount) => (
                  <Button key={amount} selected={setting === amount} icon="plus" content={amount * 3} disabled={maxSetting < amount} onClick={() => act('setting', { amount })} />
                ))}
              </LabeledList.Item>
            </LabeledList>
          </Box>
