...
const DopplerArrayContent = (props) => {
  ...
  const [activeRecordName, setActiveRecordName] = useSharedState('activeRecordrecord', records[0]?.name);
    ...
    <Section>
      <Stack>
        <Stack.Item mr={2}>
          <Tabs vertical>
            {records.map((record) => (
              <Tabs.Tab icon="file" key={record.name} selected={record.name === activeRecordName} onClick={() => setActiveRecordName(record.name)}>
                {record.name}
              </Tabs.Tab>
            ))}
          </Tabs>
              ...
              <LabeledList>
                ...
                <LabeledList.Item label="Possible Cause(s)">
                  {activeRecord.reaction_results.length
                    ? activeRecord.reaction_results.map((reaction_name) => <Box key={reaction_name}>{reaction_name}</Box>)
  ...
  return (
    <Flex direction="column" height="100%">
      <Flex.Item grow>{!records.length ? <NoticeBox>No Records</NoticeBox> : DopplerArrayRecords}</Flex.Item>
