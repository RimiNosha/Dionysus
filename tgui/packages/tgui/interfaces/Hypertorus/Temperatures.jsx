...
const VerticalBar = (props) => {
  ...
  return <div className="hypertorus-temperatures__vertical-bar">{!!value && <Box backgroundColor={color} top={`${y}px`} />}</div>;
};
...
export const HypertorusTemperatures = (props) => {
  ...
  const internal_fusion_temperature_delta = (internal_fusion_temperature - data.internal_fusion_temperature_archived) / temperature_period;
  const internal_output_temperature_delta = (internal_output_temperature - data.internal_output_temperature_archived) / temperature_period;
  const internal_coolant_temperature_delta = (internal_coolant_temperature - data.internal_coolant_temperature_archived) / temperature_period;
  const moderator_internal_temperature_delta = (moderator_internal_temperature - data.moderator_internal_temperature_archived) / temperature_period;
  ...
  if (power_level === 0) {
    ...
    next_power_level_temperature = base_max_temperature * (selected_fuel ? selected_fuel.temperature_multiplier : 1);
  }

  const temperatures = [
    prev_power_level_temperature,
    next_power_level_temperature,
    ...[internal_fusion_temperature, moderator_internal_temperature, internal_output_temperature, internal_coolant_temperature].filter((d) => d),
  ].map((d) => parseFloat(d));
    ...
    <Section title="Gas Monitoring">
      <Box className="hypertorus-temperatures__container">
        ...
        <Flex overflowY="hidden" className="hypertorus-temperatures__chart" justify="space-around">
          <TemperatureBar label="Fusion" value={internal_fusion_temperature} delta={internal_fusion_temperature_delta} color="#f2711c" />
          <TemperatureBar label="Moderator" value={moderator_internal_temperature} delta={moderator_internal_temperature_delta} color="#e03997" />
          <TemperatureBar label="Coolant" value={internal_coolant_temperature} delta={internal_coolant_temperature_delta} color="aliceblue" />
          <TemperatureBar label="Output" value={internal_output_temperature} delta={internal_output_temperature_delta} color="#20b142" />
        </Flex>
      </Box>
    </Section>
  );
