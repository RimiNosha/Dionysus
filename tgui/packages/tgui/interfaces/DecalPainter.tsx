          ...
          <Flex direction="row" wrap="nowrap" align="fill" justify="fill">
            {data.decal_list.map((decal) => {
              ...
              return nondirectional ? (
                ...
                <IconButton key={decal.decal} decal={decal.decal} dir={2} color={preview_color} label={decal.name} selected={decal.decal === data.current_decal} />
              ) : (
                ...
                <Flex key={decal.decal} direction="column" wrap="nowrap" align="fill" justify="fill">
                  {data.dir_list.map((dir) => {
                    ...
                    return <IconButton key={dir.dir} decal={decal.decal} dir={dir.dir} color={preview_color} label={`${dir.name} ${decal.name}`} selected={selected} />;
                  })}
                </Flex>
              );
            })}
...
const IconButton = (props: IconButtonParams) => {
  ...
  const generateIconKey = (decal: string, dir: number, color: string) => `${data.icon_prefix} ${decal}_${dir}_${color.replace('#', '')}`;
