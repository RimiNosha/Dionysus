 ...
 * Arguments:
 * * is_ai - boolean to decide whether the name has "Core" (AI) or JOB_CIVILLIAN (Cyborg)
 */
...
/datum/anonymous_theme/employees/anonymous_name(mob/target)
	var/is_head_of_staff = target.mind.assigned_role.departments_bitflags & DEPARTMENT_BITFLAG_COMPANY_LEADER
...
/datum/anonymous_theme/employees/anonymous_ai_name(is_ai = FALSE)
	...
	return "Employee [is_ai ? "Core" : JOB_CIVILLIAN] [verbs] [phonetic]"
